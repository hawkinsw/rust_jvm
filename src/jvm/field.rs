...
impl<'l> From<&'l Vec<u8>> for Field {
	fn from(bytes: &'l Vec<u8>) -> Self {
		...
		offset+=2;
		name_index = (bytes[offset+0] as u16) << 8 |
		             (bytes[offset+1] as u16);
		offset+=2;
		descriptor_index = (bytes[offset+0] as u16) << 8 |
		                   (bytes[offset+1] as u16);
		offset+=2;
		...
		Field{byte_len: offset, access_flags, name_index, descriptor_index, attributes_count: attributes.attributes_count(), attributes}
	}
}
...
impl<'l> From<&'l Vec<u8>> for Fields {
	fn from(bytes: &'l Vec<u8>) -> Self {
		...
		let fields_count: u16 = (bytes[offset+0] as u16) << 8 |
		                        (bytes[offset+1] as u16);
		offset+=2;

		fields = repeat(Field{..Default::default()}).take(fields_count as usize).collect();
